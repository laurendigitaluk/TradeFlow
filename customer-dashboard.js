...
async function renderSellingStatus(data,offers,acquisitions,shipping,bankDetails,completedSales=[]){
 ...
 let action='';
 const liveOffers=(Array.isArray(offers)?offers:[]).filter(o=>o.status==='published'&&o.buying_item_id===base.buying_item_id);
 if(liveOffers.length){action='<div class="customer-offer-action"><div class="offer-action-kicker">ACTION REQUIRED</div><strong>Offer received</strong><p>Choose the offer you want to accept. You can accept either the cash offer or the trade-in offer.</p>'+liveOffers.map(o=>'<div class="offer-choice" style="margin-top:12px;padding:12px;border:1px solid #d8dee5;border-radius:8px"><strong>'+esc(o.offer_mode==='trade_in'?'Trade-in offer':'Cash offer')+': '+money(o.amount,o.currency)+'</strong><textarea class="offer-response-notes" data-offer-id="'+esc(o.offer_id||o.id)+'" rows="2" placeholder="Optional response notes"></textarea><div class="actions"><button type="button" class="offer-accept status-offer-accept" data-offer-id="'+esc(o.offer_id||o.id)+'">Accept this offer</button><button type="button" class="offer-refuse" data-offer-id="'+esc(o.offer_id||o.id)+'">Refuse this offer</button></div></div>').join('')+'</div>'}
 let handoff='';
...
async function loadPortalData(){const [buying,values,offers,acq,shipping,orders,items,fulfilments,returns,addresses,sellingStatus,bankDetails,completedSales]=await Promise.all([rpc('customer_get_buying_requests'),rpc('customer_get_trading_values'),rpc('customer_get_offers_v2'),rpc('customer_get_acquisitions'),rpc('customer_get_pre_acquisition_shipping'),rpc('customer_get_orders'),rpc('customer_get_order_items'),rpc('customer_get_fulfilments'),rpc('customer_get_returns'),rpc('customer_get_addresses'),rpc('customer_get_selling_status'),rpc('customer_get_bank_details'),rpc('customer_get_completed_sales')]);profile=(await rpc('customer_get_profile'))?.[0]||null;$('customer-name').textContent=profile?`${profile.first_name||''} ${profile.last_name||''}`.trim()||'Customer':'Customer';const completedAcquisitions=Array.isArray(completedSales)?completedSales:[];
