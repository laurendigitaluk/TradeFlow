...
begin
  ...
  return query
  select o.id,o.order_reference,o.status,o.payment_status,o.currency,o.subtotal,o.shipping_total,
         o.tax_total,o.discount_total,o.total,o.amount_due,o.placed_at,
         roi.listing_id,roi.inventory_asset_id,roi.title
  ...
  where o.tenant_id=p_tenant_id and o.id=p_order_id
    and o.customer_id=v_customer_id and o.status='pending_payment';
end;
$function$;
...
create or replace function public.customer_cancel_retail_order(
  p_tenant_id uuid,p_order_id uuid,p_reason text default null
)
returns jsonb
language plpgsql
security definer
set search_path = 'pg_catalog','public'
as $function$
declare v_customer_id uuid; v_order public.retail_orders%rowtype;
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  perform private.require_tenant_feature(p_tenant_id,'module.orders');
  select c.id into v_customer_id from public.customers c
  ...
  update public.retail_orders set status='cancelled',cancelled_at=now(),updated_at=now()
  ...
  update public.payment_records pr set status='cancelled',
    processed_at=coalesce(pr.processed_at,now()),updated_at=now()
  where pr.tenant_id=p_tenant_id and pr.retail_order_id=p_order_id
  ...
  insert into public.workflow_transitions(
    ...
    p_tenant_id,'retail_order',p_order_id,v_order.status,'cancelled',auth.uid(),
    nullif(trim(p_reason),''),jsonb_build_object('source','customer_action')
  );
