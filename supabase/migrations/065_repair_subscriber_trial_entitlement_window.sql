-- Repair subscriber onboarding entitlement window
-- Applied live: 2026-09-18
--
-- New subscriber signups are created as trialing subscriptions. The capability
-- layer requires a valid trial_end for trialing subscriptions, so the signup
-- RPC must establish the same 30-day trial window used by the existing
-- subscriber test subscription.
...
set trial_end = coalesce(trial_end, started_at + interval '30 days'),
    current_period_start = coalesce(current_period_start, started_at),
    current_period_end = coalesce(current_period_end, started_at + interval '30 days'),
    updated_at = now()
