...
function offerAction(r,valuation){if(r.status!=='published')return '<span class="status-pill '+(r.status==='accepted'?'status-approved':'status-neutral')+'">'+esc(r.status==='accepted'?'Accepted by you':r.status==='refused'?'Refused':r.status)+'</span>';const cash=r.cash_amount!=null?Number(r.cash_amount):(valuation?.cash_price!=null?Number(valuation.cash_price):null);const trade=r.trade_in_amount!=null?Number(r.trade_in_amount):(valuation?.trade_in_price!=null?Number(valuation.trade_in_price):null);const offerId=esc(r.offer_id||r.id);return'<div class="offer-action-card" style="border:1px solid #c8dfce;border-radius:8px;padding:16px;background:#f4faf5"><div class="offer-action-title">Action required: choose how you want to proceed</div><p class="small">This is one offer with two available choices. Accept either the cash offer or the trade-in offer, or refuse the entire offer.</p><div class="offer-choice-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px">'+(cash!==null?'<div class="offer-choice" style="padding:14px;border:1px solid #b9d8c0;border-radius:8px;background:#fff"><strong>Cash offer</strong><div style="font-size:1.25rem;font-weight:700;margin-top:4px">'+money(cash,r.currency)+'</div><div class="actions" style="margin-top:10px"><button type="button" class="offer-accept" data-offer-id="'+offerId+'" data-offer-mode="cash">Accept cash offer</button></div></div>':'')+(trade!==null?'<div class="offer-choice" style="padding:14px;border:1px solid #b9d8c0;border-radius:8px;background:#fff"><strong>Trade-in offer</strong><div style="font-size:1.25rem;font-weight:700;margin-top:4px">'+money(trade,r.currency)+'</div><div class="actions" style="margin-top:10px"><button type="button" class="offer-accept" data-offer-id="'+offerId+'" data-offer-mode="trade_in">Accept trade-in offer</button></div></div>':'')+'</div><div style="margin-top:14px;padding-top:12px;border-top:1px solid #d8dee5"><textarea class="offer-response-notes" data-offer-id="'+offerId+'" rows="2" placeholder="Optional response notes" style="width:100%;box-sizing:border-box"></textarea><div class="actions" style="margin-top:10px"><button type="button" class="offer-refuse" data-offer-id="'+offerId+'">Refuse entire offer</button></div></div></div>'}
