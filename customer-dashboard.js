...
async function renderSellingShipping(data){
 ...
 const cards=await Promise.all(data.map(async r=>{const status=String(r.status||'').replace(/_/g,' ');const canShip=['accepted','awaiting_item','shipping'].includes(r.status);let labelUrl=r.shipping_label_url||'';if(!labelUrl&&r.shipping_label_storage_path){try{labelUrl=await storageSignedUrl(r.shipping_label_storage_path)}catch(e){console.warn('Shipping label link unavailable:',e)}}const label=labelUrl?'<a href="'+esc(labelUrl)+'" target="_blank" rel="noopener">Open / print shipping label</a><a href="'+esc(labelUrl)+'" target="_blank" rel="noopener" download style="margin-left:8px">Download shipping label</a>':'<span class="small">Shipping label not available yet.</span>';const tracking=r.shipping_tracking_number?'<p><strong>Tracking:</strong> '+esc(r.shipping_tracking_number)+(r.shipping_carrier?' · '+esc(r.shipping_carrier):'')+'</p>':'';const posted=r.posted_at?'<p><strong>Posted:</strong> '+new Date(r.posted_at).toLocaleDateString('en-GB')+'</p>':'';const instructions=r.shipping_instructions?'<p><strong>Instructions</strong><br>'+esc(r.shipping_instructions).replace(/\\n/g,'<br>')+'</p>':'<p class="small">Your subscriber will provide the shipping instructions and label here when the sale is ready for you to send the item.</p>';return '<article class="selling-handover"><div><strong>'+esc(r.acquisition_reference)+'</strong> <span class="status-pill">'+esc(status)+'</span></div><p><strong>Agreed amount:</strong> '+money(r.agreed_total,r.currency)+'</p>'+instructions+tracking+posted+'<div class="actions">'+(canShip?label:'')+(!r.posted_at&&labelUrl?'<button type="button" class="mark-posted" data-acquisition-id="'+esc(r.acquisition_id)+'">I have posted this item</button>':'')+'</div></article>'}));box.innerHTML=cards.join('');
