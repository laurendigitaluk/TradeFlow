...
function renderBasket(){
 ...
 if(!basket.length){box.innerHTML='<div class="empty">Your basket is empty.</div>';return}
 box.innerHTML=basket.map((item,i)=>'<div class="basket-row"><div><strong>'+esc(item.title||'Item')+'</strong><div class="small">'+money(item.price,item.currency)+'</div></div><button type="button" class="basket-remove" data-basket-index="'+i+'">Remove</button></div>').join('')+
 '<div class="basket-total"><strong>Total</strong><strong>'+money(basket.reduce((s,x)=>s+Number(x.price||0),0),basket[0]?.currency||'GBP')+'</strong></div><div class="actions"><button type="button" id="basket-checkout">Proceed to payment</button></div>';
}
function addToBasket(id){
 const shop=Array.isArray(window.tradeflowShopListings)?window.tradeflowShopListings:[];const item=shop.find(x=>x.listing_id===id);
 if(!item)return setMessage('That item is no longer available.','error');
 const basket=getBasket();if(basket.some(x=>x.listing_id===id))return setMessage('That item is already in your basket.','message');
 basket.push({listing_id:item.listing_id,title:item.title,price:Number(item.asking_price||0),currency:item.currency||'GBP'});
 saveBasket(basket);setMessage('Item added to your basket.','success');location.hash='#shop';
}
...
function checkout(id){addToBasket(id)}
...
const hasSellingActivity=(Array.isArray(buying)&&buying.length>0)||(Array.isArray(valuations)&&valuations.length>0)||(Array.isArray(activeOffers)&&activeOffers.length>0)||(Array.isArray(activeAcquisitions)&&activeAcquisitions.length>0)||(Array.isArray(completedAcquisitions)&&completedAcquisitions.length>0);
const sellingPanel=$('selling');if(sellingPanel)sellingPanel.hidden=!hasSellingActivity;
const sellingNav=$('selling-nav-link');if(sellingNav)sellingNav.hidden=!hasSellingActivity;
