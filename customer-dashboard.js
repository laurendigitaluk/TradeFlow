...
async function loadPortalData(){const [buying,values,offers,acq,shipping,orders,items,fulfilments,returns,shop,addresses,sellingStatus,bankDetails]=await Promise.all([rpc('customer_get_buying_requests'),rpc('customer_get_trading_values'),rpc('customer_get_offers'),rpc('customer_get_acquisitions'),rpc('customer_get_pre_acquisition_shipping'),rpc('customer_get_orders'),rpc('customer_get_order_items'),rpc('customer_get_fulfilments'),rpc('customer_get_returns'),rpc('customer_get_store_listings'),rpc('customer_get_addresses'),rpc('customer_get_selling_status'),rpc('customer_get_bank_details')]);profile=(await rpc('customer_get_profile'))?.[0]||null;$('customer-name').textContent=profile?`${profile.first_name||''} ${profile.last_name||''}`.trim()||'Customer':'Customer';const completedAcquisitions=(Array.isArray(acq)?acq:[]).filter(a=>['paid','completed'].includes(String(a.status||'').toLowerCase()));
