...
function applyWebsiteBackground(){const e=$('site-editor');if(!e)return;const mode=themeColors.background_mode==='custom'?'custom':'preset';const p=websiteBackgrounds.find(x=>x.id===themeColors.background_id)||websiteBackgrounds[13];e.dataset.backgroundMode=mode;e.style.setProperty('--site-background-color',mode==='custom'?themeColors.page_bg:p.color);e.style.setProperty('--site-background-image',mode==='custom'?'none':p.image);e.style.setProperty('--site-background-size',mode==='custom'?'cover':p.size);e.style.setProperty('--site-background-repeat',mode==='custom'?'no-repeat':p.repeat);}
function selectWebsiteBackground(id){const p=websiteBackgrounds.find(x=>x.id===id);if(!p)return;themeColors.background_id=p.id;themeColors.background_mode='preset';applyWebsiteBackground();renderEditor();markDirty();setStatus(p.label+' preset background selected. Brand colours still control the website sections. Save the draft to keep the change.','success');}
function selectPresetBackgroundMode(){themeColors.background_mode='preset';applyWebsiteBackground();renderDesignControls();renderEditor();markDirty();setStatus('Preset background mode enabled. The selected specialist background is active.','success');}
function selectCustomBackground(){themeColors.background_mode='custom';applyWebsiteBackground();renderDesignControls();renderEditor();markDirty();setStatus('Custom colour background selected. The specialist pattern is disabled until you choose a preset again.','success');}
...
function loadContent(content){
 ...
 themeColors={accent:accent,page_bg:s.theme?.page_bg||'#f5f6f8',text:s.theme?.text||'#17202a',header_bg:s.theme?.header_bg||'#ffffff',buy_bg:s.theme?.buy_bg||'#ffffff',sell_bg:s.theme?.sell_bg||'#f4f6f7',footer_bg:s.theme?.footer_bg||'#17202a',background_id:s.theme?.background_id||'clean-wave',background_mode:s.theme?.background_mode==='custom'?'custom':'preset'};
