...
async function loadPortal(){try{const ok=await profileCheck();if(!ok){saveSession(null);showAuth(true);message('This login is not registered for this business. Use Create customer account to create a new account.','error');return}showAuth(false);await loadBrand();await Promise.all([loadSelling(),loadOrders(),loadDetails(),loadCreditAccount()]);if(tradeflowPurchaseMode){try{localStorage.removeItem('tradeflow_purchase_listing_id');localStorage.removeItem('tradeflow_purchase_tenant_id')}catch{}window.dispatchEvent(new CustomEvent('tradeflow-portal-ready',{detail:{listingId:tradeflowListingId,tenantId}}))}}catch(e){showAuth(false);message(e.message||String(e),'error')}}
