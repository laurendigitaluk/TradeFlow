...
function renderSellingStatus(data){
 ...
 const cls=stage==='offer_ready'?'ready':stage==='valued'?'approved':stage==='manual_valuation'?'manual':'progress';
 const title=stage==='offer_ready'?'Offer ready — action required':stage==='valued'?'Valuation approved — offer being prepared':stage==='manual_valuation'?'Manual valuation required':'Valuation in progress';
 const message=stage==='offer_ready'?'Your valuation has been approved and an offer has been sent to you. Please review the offer below and choose Accept offer or Refuse offer.':stage==='valued'?'Your valuation has been approved. The subscriber still needs to send the offer to you.':active.message;
 box.hidden=false;
 box.className='selling-status '+cls;
 box.innerHTML='<div class="status-kicker">SELLING REQUEST</div><h3>'+esc(title)+'</h3><p><strong>'+esc(active.item_title||'Your item')+'</strong> <span class="status-pill '+(stage==='offer_ready'?'status-offer':'')+'">'+esc(stage.replace(/_/g,' '))+'</span></p><p>'+esc(message)+'</p><div class="status-meta"><span>Request <strong>'+esc(active.request_reference)+'</strong></span><span>Stage <strong>'+esc(String(stage||'').replace(/_/g,' '))+'</strong></span></div>'+(stage==='offer_ready'?'<p class="status-next action-needed"><strong>Action required:</strong> Scroll to Offers below, review the amount and choose Accept offer or Refuse offer.</p>':'')+(stage==='manual_valuation'?'<p class="status-next"><strong>What happens next:</strong> We will complete the manual valuation and show your valuation here when it is ready.</p>':'')+'</div>';
}
