-- Live definition for post-payment retail listing availability.
...
AS $function$
declare v_payment public.payment_records%rowtype; v_order public.retail_orders%rowtype; v_listing public.listings%rowtype; v_existing uuid;
begin
  ...
  if v_payment.retail_order_id is not null and p_new_status='paid' then
    ...
    where l.tenant_id=p_tenant_id and l.id=(select i.listing_id from public.retail_order_items i where i.tenant_id=p_tenant_id and i.order_id=v_order.id order by i.created_at limit 1)
    for update;
  ...
  if v_payment.retail_order_id is not null and p_new_status='paid' then
    update public.retail_orders set status='paid',payment_status='paid',paid_at=coalesce(paid_at,now()),amount_due=0,placed_at=coalesce(placed_at,now()),updated_at=now()
    ...
    if found and v_order.status in ('initiated','pending_payment') then
      update public.retail_orders set status='cancelled',cancelled_at=coalesce(cancelled_at,now()),updated_at=now() where tenant_id=p_tenant_id and id=v_order.id;
      insert into public.workflow_transitions(tenant_id,entity_type,entity_id,from_status,to_status,actor_user_id,notes,metadata)
      values(p_tenant_id,'retail_order',v_order.id,v_order.status,'cancelled',null,'External payment provider cancelled or refunded unpaid retail order',
             jsonb_build_object('source','external_payment_provider','provider',p_provider,'event_id',p_event_id));
      update public.listings l set status='published',reserved_at=null,updated_at=now()
      where l.tenant_id=p_tenant_id and l.status='reserved' and l.id in (select i.listing_id from public.retail_order_items i where i.tenant_id=p_tenant_id and i.order_id=v_order.id and i.listing_id is not null);
      update public.inventory_assets ia set status='listed',updated_at=now()
      where ia.tenant_id=p_tenant_id and ia.status='reserved' and ia.id in (select i.inventory_asset_id from public.retail_order_items i where i.tenant_id=p_tenant_id and i.order_id=v_order.id and i.inventory_asset_id is not null);
    end if;
  end if;
