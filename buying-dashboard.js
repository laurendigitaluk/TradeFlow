...
function renderParcel2GoQuotes(id,quotes){
 ...
 // Parcel2Go service buttons are handled by the document-level click delegation below.
}
async function getParcel2GoQuotes(id,b){
 ...
 }catch(e){const box=$('p2g-quotes-'+id);if(box)box.innerHTML='<div class="small" style="padding:10px;border:1px solid #e0b4b4;border-radius:6px;background:#fff5f5;color:#7a2525"><strong>Parcel2Go quote failed:</strong> '+esc(e.message||String(e))+'</div>';msg(e.message||String(e),'error')}finally{setBusy(b,false)}
}
async function createParcel2GoOrder(id,serviceCode,b){
 ...
 }catch(e){const box=$('p2g-quotes-'+id);if(box)box.innerHTML='<div class="small" style="padding:10px;border:1px solid #e0b4b4;border-radius:6px;background:#fff5f5;color:#7a2525"><strong>Parcel2Go shipment failed:</strong> '+esc(e.message||String(e))+'</div>';msg(e.message||String(e),'error')}finally{setBusy(b,false)}
}
...
function renderRequests(rows){ if(!Array.isArray(rows)||!rows.length) return '<div class="empty">No customer buying requests have been submitted to this tenant.</div>'; return '<div class="request-list">'+rows.map(r=>{const acceptedMode=['offer_accepted','awaiting_item','shipping','received','inspection','testing','repair','final_offer_required','final_offer_sent','final_offer_accepted','purchased'].includes(r.status)&&r.offer_mode;const offerSummary=acceptedMode?'<div class="request-amount" style="margin-top:6px"><strong>'+esc(r.offer_mode==='trade_in'?'Trade-in':'Cash')+': '+money(r.offer_amount,r.offer_currency||'GBP')+'</strong></div>':(r.offer_cash_amount!=null||r.offer_trade_in_amount!=null)?'<div class="request-amount" style="margin-top:6px"><div>Cash: '+(r.offer_cash_amount!=null?money(r.offer_cash_amount,r.offer_currency||'GBP'):'—')+'</div><div>Trade-in: '+(r.offer_trade_in_amount!=null?money(r.offer_trade_in_amount,r.offer_currency||'GBP'):'—')+'</div></div>':(r.offer_amount!=null?'<div class="request-amount">'+money(r.offer_amount,r.offer_currency||'GBP')+'</div>':'');return '<div class="request-card '+(r.status==='submitted'?'submitted-request-card':'')+'"><div><div class="ref">'+esc(r.request_reference)+'</div><div class="meta"><strong>'+esc(r.customer_name||'Customer')+'</strong> · Customer submission · '+(r.submitted_at?new Date(r.submitted_at).toLocaleString('en-GB'):'—')+'</div></div><div><span class="cell-label">Status</span><span class="status-pill '+requestStatusClass(r.status)+'">'+esc(requestStatusLabel(r.status))+'</span>'+offerSummary+'</div><div><span class="cell-label">Customer</span><span>'+esc(r.customer_name||'—')+'</span></div><div><span class="cell-label">Source</span><span>'+esc(r.source==='customer_portal'?'Customer portal':r.source||'—')+'</span></div><button type="button" data-action="open-request" data-id="'+esc(r.id)+'">Open request</button></div>';}).join('')+'</div>'; } 
...
async function load(){try{
 ...
 const results=await Promise.allSettled([
  ...
  api('/rest/v1/offers?select=id,buying_item_id,amount,currency,status,offer_reference,published_at,responded_at,offer_type,offer_mode&tenant_id=eq.'+encodeURIComponent(tenantId)+'&order=created_at.desc'),
 ...
 const enriched=(requests||[]).map(r=>{const i=itemByRequest[r.id],v=i&&valueByItem[i.id],o=i&&offerByItem[i.id],a=o&&acquisitionByOffer[o.id],paid=i&&paidOfferByItem[i.id],s=i&&preShippingByItem[i.id];const workflowStatus=i?.purchase_stage&&i.purchase_stage!=='none'?i.purchase_stage:(o?.status==='refused'?'offer_refused':o?.status==='published'?'offer_ready':v?'valued':r.status);return {...r,status:workflowStatus,customer_name:'—',offer_amount:paid?.offer?.amount??o?.amount??v?.cash_price??v?.amount??v?.trade_in_price??null,offer_mode:paid?.offer?.offer_mode??o?.offer_mode??null,offer_cash_amount:v?.cash_price??null,offer_trade_in_amount:v?.trade_in_price??null,offer_currency:paid?.offer?.currency??o?.currency??v?.currency??'GBP',acquisition:a||paid?.acquisition||null,preShipping:s,buying_item_id:i?.id||null,item_title:i?.title||'—',purchase_date:i?.purchased_at||paid?.acquisition?.paid_at||null,completed:Boolean((i?.purchase_stage==='purchased')||(paid?.acquisition?.status==='paid'))}});
...
async function refreshBuyingStatus(){if(!tenantId||!key||!session?.access_token)return;try{
 ...
 const enriched=(requests||[]).map(r=>{const i=itemByRequest[r.id],v=i&&valueByItem[i.id],o=i&&offerByItem[i.id],a=o&&acquisitionByOffer[o.id],existing=currentRequests.find(x=>x.id===r.id);const workflowStatus=i?.purchase_stage&&i.purchase_stage!=='none'?i.purchase_stage:(o?.status==='refused'?'offer_refused':o?.status==='published'?'offer_ready':v?'valued':r.status);return {...r,status:workflowStatus,customer_name:existing?.customer_name||'—',offer_amount:o?.amount??v?.cash_price??v?.amount??v?.trade_in_price??null,offer_mode:o?.offer_mode??null,offer_cash_amount:v?.cash_price??null,offer_trade_in_amount:v?.trade_in_price??null,offer_currency:o?.currency??v?.currency??'GBP',acquisition:a,buying_item_id:i?.id||null,completed:Boolean(i?.purchase_stage==='purchased'||a?.status==='paid')}});
...
function renderCustomerSuppliedDescription(description){
 ...
 const normalized=source.split(slashN).join('\n').replace(/\r\n/g,'\n').replace(/\r/g,'\n');
 const labels=['Product type','Manufacturer','Model','Package','Condition','Missing items','Legal right to sell'];
 ...
 labels.forEach(label=>{
  ...
  for(const nextLabel of labels){
   ...
   if(next>=0&&next<valueEnd)valueEnd=next;
  }
  pairs.push([label,normalized.slice(valueStart,valueEnd).trim()]);
  cursor=valueStart;
 });
 if(!pairs.length){
