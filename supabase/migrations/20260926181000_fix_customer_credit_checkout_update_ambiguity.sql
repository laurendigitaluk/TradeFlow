-- Qualify listing/inventory status columns because the RETURNS TABLE output
-- column "status" otherwise makes the UPDATE predicates ambiguous.
...
begin
  ...
  where l.tenant_id=p_tenant_id and l.id=v_listing.id and l.status='published';

  update public.inventory_assets as ia
  set status='sold',sold_at=coalesce(ia.sold_at,now()),updated_at=now()
  where ia.tenant_id=p_tenant_id and ia.id=v_listing.asset_id and ia.status in ('listed','reserved');
