create or replace function public.process_external_payment_event(
  p_provider text,p_event_id text,p_event_type text,p_tenant_id uuid,p_payment_id uuid,
  p_provider_payment_id text,p_new_status text,p_amount numeric,p_currency text,
...
as $function$
declare v_payment public.payment_records%rowtype; v_order public.retail_orders%rowtype; v_existing uuid;
begin
  ...
  on conflict(provider,event_id) do nothing returning id into v_existing;
  ...
  if p_provider_payment_id is not null and coalesce(v_payment.provider_payment_id,'') <> p_provider_payment_id then
  ...
  update public.payment_records set status=p_new_status,provider=p_provider,
  ...
  if v_payment.retail_order_id is not null and p_new_status in ('paid','cancelled') then
    ...
    where tenant_id=p_tenant_id and id=v_payment.retail_order_id for update;
    ...
    if p_new_status='paid' then
      ...
      update public.retail_orders set status='paid',payment_status='paid',
        paid_at=coalesce(paid_at,now()),amount_due=0,placed_at=coalesce(placed_at,now()),updated_at=now()
      ...
      where l.tenant_id=p_tenant_id and l.status='reserved'
        and l.id in (select i.listing_id from public.retail_order_items i
      ...
      where ia.tenant_id=p_tenant_id and ia.status in ('listed','reserved')
        and ia.id in (select i.inventory_asset_id from public.retail_order_items i
      ...
      where ia.tenant_id=p_tenant_id and ia.status='reserved'
        and ia.id in (select i.inventory_asset_id from public.retail_order_items i
                      where i.tenant_id=p_tenant_id and i.order_id=v_order.id and i.inventory_asset_id is not null);
    end if;
  end if;
