...
Restore branch: `checkpoint-test-two-preflight-buying-flow-20260924`
...
The customer-facing Sell to Us journey now submits the valuation request from the live website when the customer is authenticated. If authentication is required, the completed journey is carried into the customer account and submitted automatically after authentication; the customer is then taken to the account to view the request.
...
## Buying dashboard / test cleanup

The previous Pentax test request `BR-0376ED2FDE` was a new test-only submission with no valuation, offer, acquisition or inventory record. It has been closed rather than deleted, preserving the audit trail while removing it from active Buying work. The completed Canon purchase remains preserved and is now displayed under the dedicated Completed purchases section rather than the active request workspace.
