create or replace function public.customer_get_offers_v2(p_tenant_id uuid)
returns table(
  offer_id uuid,
  buying_item_id uuid,
...
revoke execute on function public.customer_get_offers_v2(uuid) from public,anon;
grant execute on function public.customer_get_offers_v2(uuid) to authenticated;
...
begin
  ...
  set status='accepted',responded_at=now(),response_notes=p_response_notes,updated_at=now()
  ...
  if v_offer_type in ('initial','revised') then
    if v_item_stage not in ('none','final_offer_refused') then
    ...
    set purchase_stage='awaiting_item',purchase_stage_updated_at=now(),updated_at=now()
    ...
    update public.buying_items
    set purchase_stage='final_offer_accepted',final_offer_accepted_at=now(),
...
as $function$
declare
  v_actor uuid:=auth.uid(); v_stage text; v_currency text; v_value_id uuid; v_offer_id uuid;
  v_customer_id uuid; v_customer_email text; v_customer_name text; v_payload jsonb;
begin
  ...
  from public.buying_items where tenant_id=p_tenant_id and id=p_buying_item_id for update;
  ...
  insert into public.trading_values(
    tenant_id,buying_item_id,method,status,amount,currency,cash_price,trade_in_price,
    confidence,calculated_at,notes,metadata
  )
  values(
    p_tenant_id,p_buying_item_id,'manual','draft',p_amount,coalesce(v_currency,'GBP'),p_amount,null,
    null,now(),coalesce(p_notes,'Final post-inspection valuation'),
  ...
  insert into public.offers(
    tenant_id,buying_item_id,trading_value_id,offer_reference,offer_type,offer_mode,status,amount,currency,created_by
  )
  values(
    p_tenant_id,p_buying_item_id,v_value_id,
    'OF-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,10)),
    'final','cash','draft',p_amount,coalesce(v_currency,'GBP'),v_actor
  )
  ...
  perform public.transition_workflow_entity(
    ...
    jsonb_build_object('source','post_inspection_final_offer',
      'inspection_id',(select id from public.buying_item_inspections
                       where tenant_id=p_tenant_id and buying_item_id=p_buying_item_id
                       order by created_at desc limit 1))
  );

  update public.buying_items set purchase_stage='final_offer_sent',
    purchase_stage_updated_at=now(),updated_at=now()
  ...
  v_payload:=jsonb_build_object(
    ...
    'offer_amount',p_amount,'currency',coalesce(v_currency,'GBP'),'offer_type','final'
  );
  ...
  if v_customer_email is not null and btrim(v_customer_email)<>'' then
    perform private.queue_customer_notification(p_tenant_id,'offer_sent','offer',v_offer_id,
