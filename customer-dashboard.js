...
async function renderSellingStatus(data,offers,acquisitions,shipping){
 ...
 let action='';
 if(liveOffer&&active.stage!=='awaiting_item'){action='<div class="customer-offer-action"><div class="offer-action-kicker">ACTION REQUIRED</div><strong>Manual offer sent: '+money(liveOffer.amount,liveOffer.currency)+'</strong><p>This offer is waiting for your response. Choose Accept offer or Refuse offer.</p><textarea class="offer-response-notes" data-offer-id="'+esc(liveOffer.offer_id)+'" rows="2" placeholder="Optional response notes"></textarea><div class="actions"><button type="button" class="offer-accept status-offer-accept" data-offer-id="'+esc(liveOffer.offer_id)+'">Accept offer</button><button type="button" class="offer-refuse" data-offer-id="'+esc(liveOffer.offer_id)+'">Refuse offer</button></div></div>'}
 let handoff='';
 if(active.stage==='awaiting_item'||active.stage==='item_on_way'){
  let labelUrl=ship?.shipping_label_url||''; if(!labelUrl&&ship?.shipping_label_storage_path){try{labelUrl=await storageSignedUrl(ship.shipping_label_storage_path)}catch(e){}}
  ...
  const tracking=ship?.shipping_tracking_url?'<a href="'+esc(ship.shipping_tracking_url)+'" target="_blank" rel="noopener">Track shipment</a>':ship?.shipping_tracking_number?'<span><strong>Tracking:</strong> '+esc(ship.shipping_tracking_number)+'</span>':'';
  const links=(labelUrl?'<a href="'+esc(labelUrl)+'" target="_blank" rel="noopener">Open / print shipping label</a>':'')+(qrUrl?'<a href="'+esc(qrUrl)+'" target="_blank" rel="noopener">Open shipping QR code</a>':'')+(tracking?tracking:'');
  handoff='<div class="shipping-handoff-summary" style="margin-top:12px;padding:12px;border:1px solid #b9d8c0;border-radius:8px;background:#f4faf5"><strong>Shipping handoff</strong><p>The subscriber has arranged the shipping and provided the information needed to send the item.</p>'+(service?'<p><strong>Service:</strong> '+esc(service)+'</p>':'')+(ship?.shipping_instructions?'<p><strong>Instructions:</strong><br>'+esc(ship.shipping_instructions).replace(/\\n/g,'<br>')+'</p>':'')+(links?'<div class="actions" style="margin-top:10px">'+links+'</div>':'')+(active.stage==='awaiting_item'?'<button type="button" class="mark-item-sent" data-acquisition-id="'+esc(acq.id)+'" style="margin-top:12px">Item sent</button>':'<p style="margin-top:12px"><strong>Item on its way.</strong> The subscriber can now monitor the shipment.</p>')+'</div>';
 }
 box.innerHTML='<div class="status-kicker">SELLING REQUEST</div><h3>'+esc(title)+'</h3><p><strong>'+esc(active.item_title||'Your item')+'</strong></p><div class="status-meta"><span>Request <strong>'+esc(active.request_reference)+'</strong></span><span>Item <strong>'+esc(active.item_reference||'—')+'</strong></span></div><p>'+esc(active.message)+'</p><div class="status-meta"><span>Stage <strong>'+esc(String(active.stage||'').replace(/_/g,' '))+'</strong></span></div>'+action+handoff+(active.stage==='manual_valuation'?'<p class="status-next"><strong>What happens next:</strong> We will complete the manual valuation and show your valuation here when it is ready.</p>':active.stage==='offer_ready'?'<p class="status-next"><strong>Action required:</strong> Review the offer below and accept or refuse it.</p>':active.stage==='offer_accepted'?'<p class="status-next"><strong>What happens next:</strong> The subscriber will provide your shipping label and instructions here when the sale is ready for you to send the item.</p>':active.stage==='awaiting_item'?'<p class="status-next"><strong>Next stage:</strong> Follow the shipping label/instructions above, send the item, then click <strong>Item sent</strong>.</p>':active.stage==='item_on_way'?'<p class="status-next"><strong>Next stage:</strong> The subscriber is now awaiting the item.</p>':'')+'</div>';
}
...
async function renderSellingShipping(data,addresses=[]){
 ...
 const cards=await Promise.all(data.map(async r=>{
  ...
  const shippingResponsibility='<p class="shipping-responsibility"><strong>Shipping arrangement:</strong> The subscriber has arranged and paid for the shipping service. Follow the shipping label and instructions provided below.</p>';
...
document.addEventListener('click',e=>{const sentButton=e.target.closest?.('.mark-item-sent');if(sentButton){e.preventDefault();e.stopPropagation();markAcquisitionPosted(sentButton.dataset.acquisitionId);return}const offerButton=e.target.closest?.('.offer-accept,.offer-refuse');if(offerButton){e.preventDefault();e.stopPropagation();respond(offerButton.dataset.offerId,offerButton.classList.contains('offer-accept')?'accept':'refuse');return}const b=e.target.closest?.('.buy-listing');if(!b)return;e.preventDefault();e.stopPropagation();checkout(b.dataset.listingId)});window.tradeflowHandleCustomerAuthSuccess=handleAuthSuccess;
