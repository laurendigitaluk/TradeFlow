  ...
  async function renderInspection(row){
    ...
    if(row.acquisition.status==='received'&&!asset){
      ...
      section.innerHTML='<div class="cell-label">PURCHASING INSPECTION</div><h2 style="margin:4px 0 8px">Inspect item before Sales</h2><p class="small">The customer record below is read-only. Compare the physical item against it and record the actual condition, discrepancies, serial/accessories and testing results.</p>'+
        ...
        '<label><strong>Inspection outcome</strong><select id="tf-outcome"><option value="">Select next route…</option><option value="ready_for_sale">Pass inspection — send to Sales</option><option value="testing_required">Send to Testing</option><option value="repair_required">Requires Repair</option><option value="not_as_described">Not as described — hold for review</option></select></label>'+
        '<div id="tf-testing-link" class="customer-info" style="margin-top:14px;display:none"><strong>Testing required</strong><p class="small">After completing this inspection, open the Testing workspace directly for this item.</p><a id="tf-open-testing" href="inventory-dashboard.html" target="_blank" rel="noopener">OPEN TESTING</a></div><label class="full"><strong>Discrepancies / missing items / faults</strong><textarea id="tf-discrepancies" rows="4" placeholder="Record anything that differs from the customer description, condition or package contents."></textarea></label>'+
      ...
      section.querySelector('#tf-outcome').onchange=()=>{const link=section.querySelector('#tf-testing-link');const a=section.querySelector('#tf-open-testing');const isTesting=section.querySelector('#tf-outcome')?.value==='testing_required';if(link)link.style.display=isTesting?'block':'none';if(a&&asset?.id)a.href='inventory-dashboard.html?status=testing&asset='+encodeURIComponent(asset.id)};
