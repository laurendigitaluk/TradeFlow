...
async function loadByTenant(){
  ...
  const rows=await api(`/rest/v1/published_site_index?select=tenant_id,hostname,revision_number,content,published_at&tenant_id=eq.${encodeURIComponent(tenantId)}&limit=1`);
