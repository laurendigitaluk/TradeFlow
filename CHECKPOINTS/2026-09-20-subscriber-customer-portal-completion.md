...
## Follow-up pass — Customer selling journey completion

Completed after the Website Builder guidance work. The customer portal's existing **Sell to us** journey now also exposes issued valuations by selling request, while retaining offer acceptance/refusal and accepted acquisitions. Accepted acquisitions show shipping instructions, carrier/tracking information when supplied, and a customer-facing shipping-label link. Customers can confirm when they have posted an item through the tenant/customer-scoped `customer_mark_acquisition_posted` RPC, which records `posted_at` and moves an eligible acquisition to `shipping`. A tenant/customer-scoped `customer_get_selling_valuations` RPC provides valuation records. Both RPCs are authenticated-only; anon execute was verified false and authenticated execute true. PR #48 merged to main with merge commit `b516cfb96af45383a5f78937eca642f83f72177c`.
