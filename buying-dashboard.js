...
async function load(){try{
 ...
 const enriched=(requests||[]).map(r=>{const i=itemByRequest[r.id],v=i&&valueByItem[i.id],o=i&&offerByItem[i.id],a=o&&acquisitionByOffer[o.id],paid=i&&paidOfferByItem[i.id],s=i&&preShippingByItem[i.id];const workflowStatus=i?.purchase_stage&&i.purchase_stage!=='none'?i.purchase_stage:(o?.status==='refused'?'offer_refused':o?.status==='published'?'offer_ready':v?'valued':r.status);return {...r,status:workflowStatus,customer_name:'—',offer_amount:paid?.offer?.amount??o?.amount??v?.cash_price??v?.amount??v?.trade_in_price??null,offer_cash_amount:v?.cash_price??null,offer_trade_in_amount:v?.trade_in_price??null,offer_currency:paid?.offer?.currency??o?.currency??v?.currency??'GBP',acquisition:a||paid?.acquisition||null,preShipping:s,buying_item_id:i?.id||null,item_title:i?.title||'—',purchase_date:i?.purchased_at||paid?.acquisition?.paid_at||null,completed:Boolean((i?.purchase_stage==='purchased')||(paid?.acquisition?.status==='paid'))}});
...
async function refreshBuyingStatus(){if(!tenantId||!key||!session?.access_token)return;try{
 ...
 const enriched=(requests||[]).map(r=>{const i=itemByRequest[r.id],v=i&&valueByItem[i.id],o=i&&offerByItem[i.id],a=o&&acquisitionByOffer[o.id],existing=currentRequests.find(x=>x.id===r.id);const workflowStatus=i?.purchase_stage&&i.purchase_stage!=='none'?i.purchase_stage:(o?.status==='refused'?'offer_refused':o?.status==='published'?'offer_ready':v?'valued':r.status);return {...r,status:workflowStatus,customer_name:existing?.customer_name||'—',offer_amount:o?.amount??v?.cash_price??v?.amount??v?.trade_in_price??null,offer_cash_amount:v?.cash_price??null,offer_trade_in_amount:v?.trade_in_price??null,offer_currency:o?.currency??v?.currency??'GBP',acquisition:a,buying_item_id:i?.id||null,completed:Boolean(i?.purchase_stage==='purchased'||a?.status==='paid')}});
