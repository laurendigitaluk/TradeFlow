...
function renderSellingStatus(data,offers){
 ...
 const cls=active.stage==='manual_valuation'?'manual':active.stage==='offer_ready'?'ready':active.stage==='offer_accepted'?'accepted':'progress';
 const title=active.stage==='manual_valuation'?'Manual valuation required':active.stage==='offer_ready'?'Manual offer sent — awaiting your response':active.stage==='offer_accepted'?'Offer accepted — awaiting shipping label':active.stage==='valued'?'Valuation completed':'Valuation in progress';
 ...
 let action='';
 if(liveOffer){action='<div class="customer-offer-action"><div class="offer-action-kicker">ACTION REQUIRED</div><strong>Manual offer sent: '+money(liveOffer.amount,liveOffer.currency)+'</strong><p>This offer is waiting for your response. Choose Accept offer or Refuse offer.</p><textarea class="offer-response-notes" data-offer-id="'+esc(liveOffer.offer_id)+'" rows="2" placeholder="Optional response notes"></textarea><div class="actions"><button type="button" class="offer-accept status-offer-accept" data-offer-id="'+esc(liveOffer.offer_id)+'">Accept offer</button><button type="button" class="offer-refuse" data-offer-id="'+esc(liveOffer.offer_id)+'">Refuse offer</button></div></div>'}
 box.innerHTML='<div class="status-kicker">SELLING REQUEST</div><h3>'+esc(title)+'</h3><p><strong>'+esc(active.item_title||'Your item')+'</strong></p><div class="status-meta"><span>Request <strong>'+esc(active.request_reference)+'</strong></span><span>Item <strong>'+esc(active.item_reference||'—')+'</strong></span></div><p>'+esc(active.message)+'</p><div class="status-meta"><span>Stage <strong>'+esc(String(active.stage||'').replace(/_/g,' '))+'</strong></span></div>'+action+(active.stage==='manual_valuation'?'<p class="status-next"><strong>What happens next:</strong> We will complete the manual valuation and show your valuation here when it is ready.</p>':active.stage==='offer_ready'?'<p class="status-next"><strong>Action required:</strong> Review the offer below and accept or refuse it.</p>':active.stage==='offer_accepted'?'<p class="status-next"><strong>What happens next:</strong> The subscriber will provide your shipping label and instructions here when the sale is ready for you to send the item.</p>':'')+'</div>';
}
...
async function renderSellingShipping(data,addresses=[]){
 ...
 const cards=await Promise.all(data.map(async r=>{
  const status=String(r.status||'').replace(/_/g,' '),canShip=['accepted','awaiting_item','shipping'].includes(r.status);
  let labelUrl=r.shipping_label_url||'';if(!labelUrl&&r.shipping_label_storage_path){try{labelUrl=await storageSignedUrl(r.shipping_label_storage_path)}catch(e){}}
  let qrUrl=r.shipping_qr_url||'';if(!qrUrl&&r.shipping_qr_storage_path){try{qrUrl=await storageSignedUrl(r.shipping_qr_storage_path)}catch(e){}}
  const automated=r.shipping_method==='automated'&&r.shipping_provider==='parcel2go';
  const method=automated?'Connected shipping — Parcel2Go':'Subscriber shipping service';
  ...
  if(automated){
   ...
   const addressOptions=addresses?.length?addresses.map(a=>'<option value="'+esc(a.id)+'" '+(a.id===defaultAddress?.id?'selected':'')+'>'+esc([a.address_type,a.line1,a.city,a.postcode].filter(Boolean).join(' · '))+'</option>').join(''):'<option value="">Add a customer address first</option>';
   if(r.shipping_payment_url){
    automatedHtml='<div data-p2g-card="'+esc(r.acquisition_id)+'" class="parcel2go-card" style="margin-top:12px;border:1px solid #cfd8df;border-radius:8px;padding:12px"><strong>Parcel2Go booking</strong><p class="small">Your shipping order has been created. Complete payment directly with Parcel2Go. The £'+esc(Number(r.agreed_total||0).toFixed(2))+' purchase offer is separate from your shipping cost.</p><div class="actions"><a href="'+esc(r.shipping_payment_url)+'" target="_blank" rel="noopener">Continue to Parcel2Go payment</a>'+(r.shipping_tracking_url?'<a href="'+esc(r.shipping_tracking_url)+'" target="_blank" rel="noopener" style="margin-left:8px">Track shipment</a>':'')+'</div></div>';
   }else{
    automatedHtml='<div data-p2g-card="'+esc(r.acquisition_id)+'" class="parcel2go-card" style="margin-top:12px;border:1px solid #cfd8df;border-radius:8px;padding:12px"><strong>Get a Parcel2Go shipping quote</strong><p class="small">Enter the parcel size and weight. Parcel2Go will show available services and prices. You pay Parcel2Go directly.</p><div class="form-grid" style="margin-top:8px"><label>Collection/delivery address<select id="p2g-address-'+esc(r.acquisition_id)+'">'+addressOptions+'</select></label><label>Weight (kg)<input id="p2g-weight-'+esc(r.acquisition_id)+'" type="number" step="0.01" min="0.01" value="'+esc(r.shipping_parcel_weight||'')+'"></label><label>Length (cm)<input id="p2g-length-'+esc(r.acquisition_id)+'" type="number" step="0.1" min="0.1" value="'+esc(r.shipping_parcel_length||'')+'"></label><label>Width (cm)<input id="p2g-width-'+esc(r.acquisition_id)+'" type="number" step="0.1" min="0.1" value="'+esc(r.shipping_parcel_width||'')+'"></label><label>Height (cm)<input id="p2g-height-'+esc(r.acquisition_id)+'" type="number" step="0.1" min="0.1" value="'+esc(r.shipping_parcel_height||'')+'"></label><label>Preferred collection date<input id="p2g-date-'+esc(r.acquisition_id)+'" type="date"></label></div><div class="actions" style="margin-top:10px"><button type="button" data-p2g-quote="'+esc(r.acquisition_id)+'">Get shipping quote</button></div><div class="p2g-results">'+renderParcelQuoteOptions(r.acquisition_id)+'</div></div>';
   }
  }
  return '<article class="selling-handover"><div><strong>'+esc(r.acquisition_reference)+'</strong> <span class="status-pill">'+esc(status)+'</span></div><p><strong>Agreed amount:</strong> '+money(r.agreed_total,r.currency)+'</p><p><strong>Shipping method:</strong> '+esc(method)+'</p>'+shippingResponsibility+service+instructions+tracking+qr+posted+automatedHtml+'<div class="actions">'+(!automated&&canShip?label:'')+(!r.posted_at&&(labelUrl||qrUrl)?'<button type="button" class="mark-posted" data-acquisition-id="'+esc(r.acquisition_id)+'">I have posted this item</button>':'')+'</div></article>';
 }));
...
async function loadPortalData(){
 const names=['buying','values','offers','acq','shipping','orders','items','fulfilments','returns','shop','addresses','sellingStatus'];
 const calls=['customer_get_buying_requests','customer_get_trading_values','customer_get_offers','customer_get_acquisitions','customer_get_acquisition_shipping','customer_get_orders','customer_get_order_items','customer_get_fulfilments','customer_get_returns','customer_get_store_listings','customer_get_addresses','customer_get_selling_status'];
 const settled=await Promise.allSettled(calls.map(name=>rpc(name)));
 const data={};const failures=[];
 settled.forEach((result,i)=>{if(result.status==='fulfilled')data[names[i]]=result.value;else{data[names[i]]=[];failures.push(names[i]+': '+(result.reason?.message||String(result.reason)))}});

 const buying=data.buying,values=data.values,offers=data.offers,acq=data.acq,shipping=data.shipping,orders=data.orders,items=data.items,fulfilments=data.fulfilments,returns=data.returns,shop=data.shop,addresses=data.addresses,sellingStatus=data.sellingStatus;
 try{profile=(await rpc('customer_get_profile'))?.[0]||null}catch(e){profile=null;failures.push('customer_get_profile: '+(e.message||String(e)))}
 $('customer-name').textContent=profile?((profile.first_name||'')+' '+(profile.last_name||'')).trim()||'Customer':'Customer';
 $('buying-count').textContent=buying?.length||0;$('order-count').textContent=orders?.length||0;$('return-count').textContent=returns?.length||0;
 $('buying-list').innerHTML=rows(buying,[{key:'request_reference',label:'Reference'},{key:'status',label:'Status'},{key:'source',label:'Source'}],'No selling requests yet.');
 renderSellingStatus(sellingStatus,offers);
 try{renderSellingValuations(await rpc('customer_get_selling_valuations'))}catch(e){$('valuation-list').innerHTML='<div class="empty">Valuations could not be loaded.</div>';failures.push('customer_get_selling_valuations: '+(e.message||String(e)))}
 ...
 $('acquisition-list').innerHTML=rows(acq,[{key:'acquisition_reference',label:'Reference'},{key:'status',label:'Status'},{key:'agreed_total',label:'Agreed',render:r=>money(r.agreed_total,r.currency)}],'No accepted sales yet.');
 try{await renderSellingShipping(shipping,addresses)}catch(e){$('selling-shipping').innerHTML='<div class="empty">Shipping information could not be loaded.</div>';failures.push('shipping rendering: '+(e.message||String(e)))}
 $('order-list').innerHTML=rows(orders,[{key:'order_reference',label:'Order'},{key:'status',label:'Status'},{key:'total',label:'Total',render:r=>money(r.total,r.currency)},{key:'payment_status',label:'Payment',render:r=>r.status==='pending_payment'?'Awaiting payment':esc(r.payment_status)},{key:'placed_at',label:'Placed',render:r=>r.placed_at?new Date(r.placed_at).toLocaleDateString('en-GB'):'—'},{key:'order_id',label:'Action',render:r=>r.status==='pending_payment'?'<button type="button" data-pay-order-id="'+esc(r.order_id)+'">Pay now</button>':'—'}],'No orders yet.');
 $('shop-list').innerHTML=rows(shop,[{key:'listing_reference',label:'Listing'},{key:'title',label:'Item'},{key:'asking_price',label:'Price',render:r=>money(r.asking_price,r.currency)},{key:'listing_id',label:'Action',render:r=>'<button type="button" class="buy-listing" data-listing-id="'+esc(r.listing_id)+'">Buy</button>'}],'No published items currently available.');
 $('valuation-summary').textContent=values?.length?values.length+' valuation record(s) available.':'No approved valuation is currently available.';
 if(profile){$('profile-first-name').value=profile.first_name||'';$('profile-last-name').value=profile.last_name||'';$('profile-email').value=profile.email||'';$('profile-phone').value=profile.phone||'';$('profile-list').textContent=''}else{$('profile-list').textContent='Profile not available.'}
 $('address-list').innerHTML=rows(addresses,[{key:'address_type',label:'Type'},{key:'line1',label:'Address'},{key:'city',label:'City'},{key:'postcode',label:'Postcode'}],'No saved addresses.');
 renderCustomerFulfilments(fulfilments);renderCustomerReturns(returns,items,orders);document.querySelectorAll('.buy-listing').forEach(b=>b.onclick=null);document.querySelectorAll('[data-pay-order-id]').forEach(b=>b.onclick=()=>payOrder(b.dataset.payOrderId));
 try{await loadCategories()}catch(e){failures.push('customer_get_buying_categories: '+(e.message||String(e))}
 restoreSellingJourney();
 if(failures.length){console.error('TradeFlow customer portal data-load failures:',failures);setMessage('Customer portal loaded with some data sources unavailable: '+failures.join(' | '),'error')}
}
...
async function handleAuthSuccess(data){try{saveSession(data);await ensureCustomerRegistration();await initialisePortal()}catch(err){showAuth(false);setMessage(err.message||String(err),'error')}}
