...
async function load(){try{
 ...
 const customerRows=await Promise.all((requests||[]).map(async r=>{try{return [r.id,(await api('/rest/v1/customers?select=first_name,last_name&tenant_id=eq.'+encodeURIComponent(tenantId)+'&id=eq.'+encodeURIComponent(r.customer_id)))?.[0]||null]}catch{return [r.id,null]}}));
 const customerByRequest=Object.fromEntries(customerRows);
 const enriched=(requests||[]).map(r=>{const i=itemByRequest[r.id],v=i&&valueByItem[i.id],o=i&&offerByItem[i.id],c=customerByRequest[r.id],a=o&&acquisitionByOffer[o.id],paid=i&&paidOfferByItem[i.id],s=i&&preShippingByItem[i.id];const workflowStatus=i?.purchase_stage&&i.purchase_stage!=='none'?i.purchase_stage:(o?.status==='refused'?'offer_refused':o?.status==='published'?'offer_ready':v?'valued':r.status);return {...r,status:workflowStatus,customer_name:[c?.first_name,c?.last_name].filter(Boolean).join(' ')||'—',offer_amount:paid?.offer?.amount??o?.amount??v?.cash_price??v?.amount??v?.trade_in_price??null,offer_currency:paid?.offer?.currency??o?.currency??v?.currency??'GBP',acquisition:a||paid?.acquisition||null,preShipping:s,buying_item_id:i?.id||null,item_title:i?.title||'—',purchase_date:i?.purchased_at||paid?.acquisition?.paid_at||null,completed:Boolean((i?.purchase_stage==='purchased')||(paid?.acquisition?.status==='paid'))}}); 
 const completedPurchases=enriched.filter(r=>r.completed);
 const activeRequests=enriched.filter(r=>!r.completed&&r.status!=='closed');
...
async function refreshBuyingStatus(){if(!tenantId||!key||!session?.access_token)return;try{
 ...
 const activeRequests=enriched.filter(r=>!r.completed&&r.status!=='closed');currentRequests=activeRequests;$('request-list').innerHTML=renderRequests(activeRequests);if($('request-count'))$('request-count').textContent=activeRequests.length+' active request'+(activeRequests.length===1?'':'s');document.querySelectorAll('#request-list button[data-id]').forEach(btn=>btn.addEventListener('click',()=>showRequest(btn.dataset.id,currentRequests)));if(openRequestId){const latest=activeRequests.find(x=>x.id===openRequestId);if(latest)updateOpenRequestStatus(latest);}
...
function renderItem(i,request,customer){
 ...
 const workflowGrid='<div class="workflow-grid"><div class="workflow-box"><h4>Catalogue & pricing</h4><div id="match-'+i.id+'" class="small">'+(i.buying_product_id?'Catalogue product linked.':'No subscriber catalogue product linked yet. Manual valuation is available below.')+'</div><label style="display:block;margin:10px 0">Condition<select id="condition-'+i.id+'" style="width:100%;padding:8px"><option value="">Select condition</option><option value="sealed" '+(i.item_condition==='sealed'?'selected':'')+'>Sealed</option><option value="opened_never_used" '+(i.item_condition==='opened_never_used'?'selected':'')+'>Opened, Never Used</option><option value="excellent" '+(i.item_condition==='excellent'?'selected':'')+'>Excellent</option><option value="good" '+(i.item_condition==='good'?'selected':'')+'>Good</option><option value="poor" '+(i.item_condition==='poor'?'selected':'')+'>Poor</option></select></label><div id="configured-price-'+i.id+'" class="small">Select the condition to calculate the configured buying price.</div><button type="button" data-action="calculate" data-id="'+i.id+'" style="margin-top:8px">Calculate automatic price</button></div><div class="workflow-box"><h4>Valuation</h4><div id="valuation-'+i.id+'" class="small">Loading current valuation…</div><div class="money-grid" style="grid-template-columns:1fr 1fr;gap:12px"><div style="border:1px solid #d8dee5;border-radius:8px;padding:12px;background:#fff"><strong>Buying Offer</strong><div class="small" style="margin:4px 0 8px">Cash purchase price offered to the customer.</div><input id="cash-'+i.id+'" type="number" min="0" step="0.01" placeholder="Buying offer (£)" style="width:100%"></div><div style="border:1px solid #d8dee5;border-radius:8px;padding:12px;background:#fff"><strong>Trade-In Offer</strong><div class="small" style="margin:4px 0 8px">Trade-in value offered against a retail purchase.</div><input id="trade-'+i.id+'" type="number" min="0" step="0.01" placeholder="Trade-in offer (£)" style="width:100%"></div></div><div style="margin-top:10px"><input id="confidence-'+i.id+'" type="number" min="0" max="1" step="0.01" placeholder="Confidence 0–1" style="width:100%"></div><input id="value-notes-'+i.id+'" style="width:100%;margin-top:8px" placeholder="Valuation notes"><button type="button" data-action="value" data-id="'+i.id+'" style="margin-top:8px">Create & approve valuation</button></div><div class="workflow-box"><h4>Offer</h4><div id="offer-'+i.id+'" class="small">Loading current offer…</div><div id="offer-status-'+i.id+'" class="small" style="margin-top:8px">Automatic pricing takes priority when an automatic price is available. Otherwise enter the manual cash and trade-in offers below.</div><div class="money-grid" style="grid-template-columns:1fr 1fr;gap:12px;margin-top:10px"><div style="border:1px solid #d8dee5;border-radius:8px;padding:12px;background:#fff"><strong>Manual offer — Cash</strong><div class="small" style="margin:4px 0 8px">Cash purchase amount offered to the customer.</div><input id="offer-cash-'+i.id+'" type="number" min="0" step="0.01" placeholder="Cash offer (£)" style="width:100%"></div><div style="border:1px solid #d8dee5;border-radius:8px;padding:12px;background:#fff"><strong>Manual offer — Trade-in</strong><div class="small" style="margin:4px 0 8px">Trade-in credit offered against a retail purchase.</div><input id="offer-trade-'+i.id+'" type="number" min="0" step="0.01" placeholder="Trade-in offer (£)" style="width:100%"></div></div><button type="button" data-action="offer" data-id="'+i.id+'" style="margin-top:8px">Send manual offer to customer</button></div></div>
 return '<article class="item-workspace"><div class="item-header"><div><h3>'+esc(i.title||'Untitled')+'</h3><p><span class="cell-label">Item reference</span> '+esc(i.item_reference)+' · Customer: <strong>'+esc(customerName)+'</strong> · Status: <strong>'+esc(i.status)+'</strong> · Purchase stage: <strong>'+esc(i.purchase_stage||'none').replace(/_/g,' ')+'</strong> · Quantity: '+esc(i.quantity)+'</p></div><div class="actions">'+nextButtons(i)+'</div></div><section class="customer-review-stage"><div class="section-head"><div><h4>Review customer submission</h4><p class="small">Check the complete customer submission before moving it into valuation and offer review.</p></div><span class="status-pill '+requestStatusClass(i.status)+'">'+esc(requestStatusLabel(i.status))+'</span></div><div class="customer-contact-grid"><div><span>Customer</span><strong>'+esc(customerName)+'</strong></div><div><span>Customer reference</span><strong>'+esc(customer?.customer_reference||'—')+'</strong></div><div><span>Email</span><strong>'+esc(customer?.email||'—')+'</strong></div><div><span>Phone</span><strong>'+esc(customer?.phone||'—')+'</strong></div></div><div class="customer-supplied"><h4>Customer supplied product details</h4><p class="small"><strong>Everything below is information supplied by the customer. Use it when checking the item and setting the valuation.</strong></p><div class="field-grid customer-general-details"><div><span class="cell-label">Condition</span><strong>'+esc(i.item_condition||'Not supplied')+'</strong></div><div><span class="cell-label">Quantity</span><strong>'+esc(i.quantity??'1')+'</strong></div></div><div class="supplied-text">'+esc(i.description||'No additional item description was supplied.')+'</div>'+(requestNotes?'<div class="supplied-request-notes"><span class="cell-label">Customer request notes</span><div class="supplied-text">'+esc(requestNotes)+'</div></div>':'')+'<div id="fields-'+i.id+'" class="small" style="margin-top:10px">Loading customer-supplied fields…</div></div><div id="photos-'+i.id+'" class="customer-photo-review"><h4>Photographs</h4><div class="small">Loading photographs…</div></div></section>'+ (submitted?'<section class="workflow-next-step"><h4>Review stage</h4><p>Check the customer details, supplied product information and photographs above. When everything is ready, move the item to review.</p><div class="actions">'+nextButtons(i)+'</div></section>':'<section class="valuation-offer-stage"><div class="section-head"><div><h4>Valuation & offer stage</h4><p class="small">The submission has been moved into review. Check the item and set the valuation, then create and publish the offer.</p></div></div>'+workflowGrid+'</section>')+'<div id="evidence-'+i.id+'" class="customer-info" style="margin-top:16px"><h4 style="margin:0 0 8px">Research evidence</h4><div class="small">Loading evidence…</div></div></article>';
}
...
async function loadItemFinancials(itemId,requestStatus=''){
 try{
  const values=await api('/rest/v1/trading_values?select=id,method,status,amount,currency,cash_price,trade_in_price,confidence,approved_at,notes&tenant_id=eq.'+encodeURIComponent(tenantId)+'&buying_item_id=eq.'+encodeURIComponent(itemId)+'&order=created_at.desc');
  ...
  const automaticActive=(offers||[]).some(o=>o.status==='published'&&o.offer_type==='initial'&&automaticValueIds.has(o.trading_value_id));
  $('valuation-'+itemId).innerHTML=values?.length?values.map(v=>'<div><strong>'+esc(v.status)+'</strong> — cash '+money(v.cash_price)+' / trade-in '+money(v.trade_in_price)+' · '+esc(v.method)+(v.approved_at?' · approved':'')+'</div>').join(''):'No valuation yet.';
  $('offer-'+itemId).innerHTML=offers?.length?offers.map(o=>'<div class="offer-detail-card"><div><strong>'+esc(o.offer_mode==='trade_in'?'Trade-in offer':'Cash offer')+'</strong> <span class="status-pill status-offer">'+(o.status==='published'?'Offer sent — awaiting customer':o.status==='accepted'?'Offer accepted by customer':o.status==='refused'?'Offer refused':o.status==='superseded'?'Offer superseded':'Offer '+esc(o.status))+'</span></div><div class="offer-amount"><strong>'+money(o.amount,o.currency)+'</strong></div><div class="small">Offer type: '+esc(o.offer_type)+' · Customer response: '+(o.status==='published'?'Awaiting acceptance or refusal.':esc(o.status))+'</div></div>').join(''):(approved?'<div class="offer-detail-card pending-offer"><strong>Approved valuation: '+money(approved.cash_price??approved.amount)+'</strong><div class="small">No initial offer has been sent yet. Send the manual cash/trade-in offer below.</div></div>':'No offer yet.');
  ...
  if(automaticActive){
    if(statusBox)statusBox.innerHTML='<strong>Automatic pricing is active.</strong> The automatic cash/trade-in pricing overrides manual initial offers.';
    if(cashInput)cashInput.disabled=true;
    if(tradeInput)tradeInput.disabled=true;
    if(sendButton)sendButton.disabled=true;
  }else if(statusBox){
    statusBox.textContent='No automatic initial offer is active. You can send the approved manual cash and/or trade-in offer to the customer.';
  }
...
async function createValuation(itemId,b){
 const cash=parseFloat($('cash-'+itemId).value);
 const trade=parseFloat($('trade-'+itemId).value);
 const confidence=$('confidence-'+itemId).value===''?null:parseFloat($('confidence-'+itemId).value);
 const notes=$('value-notes-'+itemId).value.trim();
 if(!Number.isFinite(cash)&&!Number.isFinite(trade))return msg('Enter a cash price or trade-in price.','error');
 if(confidence!==null&&(confidence<0||confidence>1))return msg('Confidence must be between 0 and 1.','error');
 setBusy(b,true);
 try{
  const activeAuto=await api('/rest/v1/trading_values?select=id&tenant_id=eq.'+encodeURIComponent(tenantId)+'&buying_item_id=eq.'+encodeURIComponent(itemId)+'&method=eq.automatic&status=eq.approved&limit=1');
  if(activeAuto?.length){setBusy(b,false);return msg('Automatic pricing is active for this item and overrides manual valuation.','error')}
  const amount=Number.isFinite(cash)?cash:(trade??0);
  const rows=await api('/rest/v1/trading_values',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({tenant_id:tenantId,buying_item_id:itemId,method:'manual',status:'draft',amount,currency:'GBP',cash_price:Number.isFinite(cash)?cash:null,trade_in_price:Number.isFinite(trade)?trade:null,confidence,calculated_at:new Date().toISOString(),notes:notes||null,metadata:{source:'subscriber_buying_dashboard'}})});
  const v=Array.isArray(rows)?rows[0]:rows;
  await transition('trading_value',v.id,'draft','approved',notes);
  ...
  if(itemStatus==='under_review')await transition('buying_item',itemId,'under_review','valued');
  else if(itemStatus==='submitted')await transition('buying_item',itemId,'submitted','under_review').then(()=>transition('buying_item',itemId,'under_review','valued'));
  ...
  if(requestId){
    ...
    if(rs==='under_review')await transition('buying_request',requestId,'under_review','valued');
    else if(rs==='submitted')await transition('buying_request',requestId,'submitted','under_review').then(()=>transition('buying_request',requestId,'under_review','valued'));
  }
  await loadItemFinancials(itemId);
