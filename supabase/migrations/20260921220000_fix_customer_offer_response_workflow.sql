...
declare v_customer_id uuid;v_buying_item_id uuid;v_offer_amount numeric;v_currency text;v_offer_status text;v_expires_at timestamptz;v_acquisition_id uuid;v_now timestamptz:=now();v_ref text;
begin
 perform private.require_tenant_feature(p_tenant_id,'module.offers');
 if auth.uid() is null then raise exception 'authentication required'; end if;
 v_customer_id:=private.customer_id_for_current_user(p_tenant_id); if v_customer_id is null then raise exception 'customer account not linked to tenant'; end if;
 ...
 insert into public.workflow_transitions(tenant_id,entity_type,entity_id,from_status,to_status,actor_user_id,notes,metadata) values(p_tenant_id,'offer',p_offer_id,'published','accepted',auth.uid(),p_response_notes,'{"source":"customer_action"}'::jsonb);
 v_ref:='ACQ-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,12));
 insert into public.acquisitions(tenant_id,acquisition_reference,status,customer_id,source_offer_id,currency,agreed_total,payment_total,accepted_at,created_by) values(p_tenant_id,v_ref,'accepted',v_customer_id,p_offer_id,v_currency,v_offer_amount,v_offer_amount,v_now,auth.uid()) returning id into v_acquisition_id;
 insert into public.acquisition_items(tenant_id,acquisition_id,buying_item_id,offer_id,status,agreed_amount,currency) values(p_tenant_id,v_acquisition_id,v_buying_item_id,p_offer_id,'accepted',v_offer_amount,v_currency);
...
begin
 perform private.require_tenant_feature(p_tenant_id,'module.offers'); if auth.uid() is null then raise exception 'authentication required'; end if;
 v_customer_id:=private.customer_id_for_current_user(p_tenant_id); if v_customer_id is null then raise exception 'customer account not linked to tenant'; end if;
 ...
 insert into public.workflow_transitions(tenant_id,entity_type,entity_id,from_status,to_status,actor_user_id,notes,metadata) values(p_tenant_id,'offer',p_offer_id,'published','refused',auth.uid(),p_response_notes,'{"source":"customer_action"}'::jsonb);
 return true;
end;$function$;