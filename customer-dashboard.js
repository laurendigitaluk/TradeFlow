...
async function renderSellingShipping(data,addresses=[]){
 ...
 const cards=await Promise.all(data.map(async r=>{
  ...
  const method=automated?'Integrated shipping — '+(r.shipping_provider||'Connected provider'):'Manual shipping handoff';
  ...
  let automatedHtml='';
  if(automated){
    const providerLabel=String(r.shipping_provider||'Integrated shipping').replace(/_/g,' ');
    automatedHtml='<div class="parcel2go-card" style="margin-top:12px;border:1px solid #cfd8df;border-radius:8px;padding:12px"><strong>'+esc(providerLabel)+'</strong><p class="small">The subscriber has arranged this shipping service for you. Use the shipping label, QR code, link and instructions supplied above. You do not need to arrange or pay for the shipping.</p>'+(r.shipping_tracking_url?'<div class="actions" style="margin-top:8px"><a href="'+esc(r.shipping_tracking_url)+'" target="_blank" rel="noopener">Track shipment</a></div>':'')+'</div>';
  }
  return '<article class="selling-handover"><div><strong>'+esc(r.acquisition_reference)+'</strong> <span class="status-pill">'+esc(status)+'</span></div><p><strong>Agreed amount:</strong> '+money(r.agreed_total,r.currency)+'</p><p><strong>Shipping method:</strong> '+esc(method)+'</p>'+shippingResponsibility+service+instructions+tracking+qr+posted+automatedHtml+'<div class="actions">'+(!automated&&canShip?label:'')+(!r.posted_at&&(labelUrl||qrUrl)?'<button type="button" class="mark-posted" data-acquisition-id="'+esc(r.acquisition_id)+'">Item sent</button>':'')+'</div></article>';
 }));
