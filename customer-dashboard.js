const tradeflowParams=new URLSearchParams(location.search);
...
const tradeflowStoredListingId=localStorage.getItem('tradeflow_purchase_listing_id')||'';
const tradeflowListingId=tradeflowParams.get('listing_id')||(tradeflowFromProduct?(tradeflowReferrer.searchParams.get('listing')||tradeflowReferrer.searchParams.get('listing_id')):'')||(tradeflowParams.get('purchase')==='1'?tradeflowStoredListingId:'');
const tradeflowPurchaseMode=!!tradeflowListingId;
const SUPABASE_URL='https://twfbmjwwqzxdxvclxbun.supabase.co';
const KEY='sb_publishable_AvcMgtUKV0O5k8H6k94mZQ_qH4pEIS9';
const SESSION_STORAGE='tradeflow_customer_session';
let key=KEY,session=null,tenantId=new URLSearchParams(location.search).get('tenant_id')||localStorage.getItem('tradeflow_purchase_tenant_id')||localStorage.getItem('tradeflow_customer_tenant_id'),profile=null;
...
async function loadPortal(){try{const ok=await profileCheck();if(!ok){saveSession(null);showAuth(true);message('This login is not registered for this business. Use Create customer account to create a new account.','error');return}showAuth(false);await loadBrand();if(tradeflowPurchaseMode){try{localStorage.removeItem('tradeflow_purchase_listing_id');localStorage.removeItem('tradeflow_purchase_tenant_id')}catch{}window.dispatchEvent(new CustomEvent('tradeflow-portal-ready',{detail:{listingId:tradeflowListingId,tenantId}}));return}await Promise.all([loadSelling(),loadOrders(),loadDetails(),loadCreditAccount()]);}catch(e){showAuth(false);message(e.message||String(e),'error')}}
